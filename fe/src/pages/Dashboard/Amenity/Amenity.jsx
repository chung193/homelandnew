...
import { bulkDestroy, getAll, storage, update } from './AmenityServices';
import getColumns from './AmenityColumns';
import { getBreadcrumbs } from './AmenityBreadcrumb';
import AmenityAddModal from './AmenityAddModal';

const STORAGE_KEY = 'list-view-options:amenity';
...
const Amenity = () => {
    ...
    const handleDelete = () => {
        showConfirm(t('pages.amenity.deleteConfirmTitle'), t('pages.amenity.deleteConfirmMessage', { count: selectedRows.size }), doDelete, closeModal);
    };
    ...
    return (
        <MainCard pageTitle={t('pages.amenity.title')} pageDescription={t('pages.amenity.description')} breadcrumbs={breadcrumbs}>
            <MetaData title="Amenity Management" description="Amenity management page" />
            <Toolbar
                loadData={loadData}
                handleAdd={() => openModal(t('pages.amenity.addAmenity'), <AmenityAddModal onSubmit={addAmenity} onClose={closeModal} />)}
