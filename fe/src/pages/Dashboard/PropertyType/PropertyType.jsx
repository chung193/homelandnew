...
import { bulkDestroy, getAll, storage, update } from './PropertyTypeServices';
import getColumns from './PropertyTypeColumns';
import { getBreadcrumbs } from './PropertyTypeBreadcrumb';
import PropertyTypeAddModal from './PropertyTypeAddModal';

const STORAGE_KEY = 'list-view-options:property-type';
...
const PropertyType = () => {
    ...
    const addPropertyType = (data) => {
        ...
        storage(data)
            .then(() => {
                showNotification('Thêm loại bất động sản thành công', 'success');
                loadData();
                closeModal();
            })
            .catch((err) => showNotification(err.response?.data?.message || err.message, 'error'))
    ...
    const handleDelete = () => {
        showConfirm(t('pages.propertyType.deleteConfirmTitle'), t('pages.propertyType.deleteConfirmMessage', { count: selectedRows.size }), doDelete, closeModal);
    };
    ...
    return (
        <MainCard pageTitle={t('pages.propertyType.title')} pageDescription={t('pages.propertyType.description')} breadcrumbs={breadcrumbs}>
            <MetaData title="Property Type Management" description="Property type management page" />
            <Toolbar
                loadData={loadData}
                handleAdd={() => openModal(t('pages.propertyType.addPropertyType'), <PropertyTypeAddModal onSubmit={addPropertyType} onClose={closeModal} />)}
